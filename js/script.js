 	...
 	function posts(beginning, end) {
		...
		$.getJSON("https://jsonplaceholder.typicode.com/posts", function(dataProcessed) {
			...
			$(".comment-btn").click(function () {
				...
				// .next() == .commentsBox
				if ($(this).next().hasClass("switch")) { // OPEN
					comments(postId);
 	...
 	function comments(post) {
 		...
 		$.getJSON(link, function(dataProcessed) {
			...
			for (var i = 0; i < dataProcessed.length; i++) {
				...
				$("#"+post+"").next().append("<div class='comment animate__animated animate__zoomIn'><h6 style='color: cadetblue;'>"+email+"</h6><strong>"+name+"</strong><br><p>"+body+"</p></div>");
			}
		});
 	}
